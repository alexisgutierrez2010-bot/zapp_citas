...
foreach ($usuarios_encontrados as $usuario) {
    // 2. Generar nueva contraseña temporal para CADA usuario encontrado
    ...
    try {
        ...
        $mail->Body    = '
            <html><body>
                ...
                <p>Se ha solicitado recuperar la contraseña para el negocio <strong>' . htmlspecialchars($usuario['nombre_negocio']) . '</strong>.</p>
        ...
        $emails_enviados[] = $usuario['correo_electronico'];

    } catch (Exception $e) {
        // Si falla uno, continuamos con los otros, pero logueamos el error
        error_log("Error recuperando clave para usuario {$usuario['id_usuario']}: " . $e->getMessage());
    }
}

if (count($emails_enviados) > 0) {
    echo json_encode([
        'message' => 'Instrucciones enviadas con éxito.',
        'email_sent_to' => implode(', ', array_unique($emails_enviados))
    ]);
} else {
    http_response_code(500);
    echo json_encode(['error' => "No se pudo enviar ningún correo de recuperación."]);
}