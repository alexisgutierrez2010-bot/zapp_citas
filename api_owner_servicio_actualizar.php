...
// Detectar si es JSON (para activar/desactivar) o FormData (para editar con foto)
...
// Procesar Imagen a BLOB si se subió una nueva
if (isset($_FILES['foto_servicio']) && $_FILES['foto_servicio']['error'] === UPLOAD_ERR_OK) {
    ...
    $sql = "UPDATE j104_servicios SET nombre_servicio = ?, duracion_valor = ?, duracion_unidad = ?, precio = ?, activo = ?, foto_servicio = ? WHERE id_servicio = ? AND id_negocio = ?";
    $stmt = $conn->prepare($sql);
    $null_val = NULL;
    $stmt->bind_param("sisdibii", $nombre_servicio, $duracion_valor, $duracion_unidad, $precio, $activo, $null_val, $id_servicio, $id_negocio_session);
